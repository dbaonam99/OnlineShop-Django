    ...
    useEffect(() => {
        if (products.length > 0 && order.length > 0) {
            const allCate = Object.values(
                products.reduce((a, { sex }) => {
                    a[sex] = a[sex] || { sex, count: 0 }
                    return a
                }, Object.create(null))
            )
            ...
            for (let i in allCate) {
                for (let j in products) {
                    if (allCate[i].sex === products[j].sex) {
                        allCate[i].count += products[j].sold
                    }
                }
            }
            ...
            for (let i in topCate) {
                topCateList.push(topCate[i].sex)
            }
            setData({
                labels: topCateList,
                datasets: [
