...
export default function DashboardMain() {
    ...
    useEffect(() => {
        ...
        axios.get(`http://127.0.0.1:8000/users/list`).then((res) => {
            setUser(res.data)
        })
        axios.get(`http://127.0.0.1:8000/email`).then((res) => {
            setEmail(res.data)
        })
        axios.get(`http://127.0.0.1:8000/order`).then((res) => {
            ...
            for (let i in res.data) {
                if (
                    new Date(res.data[i].orderDate).getMonth() + 1 ===
                        currentMonth &&
                    new Date(res.data[i].orderDate).getFullYear() ===
                        currentYear
                ) {
