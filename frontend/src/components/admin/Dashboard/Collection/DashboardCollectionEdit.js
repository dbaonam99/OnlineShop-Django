...
export default function DashboardCollectionEdit(props) {
    ...
    useEffect(() => {
        ...
        if (collection) {
            ;(async () => {
                setCollectionBanner([collection.banner])
                setCollectionName(collection.name)
                const a = [...productList]
                for (let i in collection.products) {
                    const res = await axios.get(
                        `http://localhost:8000/api/products/${collection.products[i]}/?format=json`
                    )
                    a.push(res.data)
                }
                setProductList(a)
    ...
    const onSubmit = (event) => {
        ...
        if (file.length > 0) {
            ...
            imageArr.forEach((image) => {
                ...
                axios
                    ...
                    .then((res) => {
                        ...
                        axios
                            .put(
                                `http://localhost:8000/api/collections/${collection.id}/`,
                                data
                            )
                            .then(() => {
            ...
            const data = {
                banner: props.collection.banner,
                products: productList.map((item) => item.id),
                name: collectionName,
            }
            axios
