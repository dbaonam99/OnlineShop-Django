...
export default function DashboardCollectionTable(props) {
    ...
    const deleteOnClick = (event) => {
        axios
            .delete(`http://127.0.0.1:8000/api/collections/${event.target.id}/`)
            .then(() => {
    ...
    const searchOnChange = (event) => {
        ...
        for (let i in constCollection) {
            if (constCollection[i].name.toLowerCase().includes(searchInput)) {
        ...
        if (event.target.id === 'CollectionName') {
            if (isSortByName) {
                ...
                sortByName.sort(function (a, b) {
                    var collectionA = a.name.toLowerCase()
                    var collectionB = b.name.toLowerCase()
